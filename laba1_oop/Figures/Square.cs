...
namespace laba1_oop
{
    ...
    {
        public Square(Color color, float width) : base(color, width) { }

        public Square(Color color, float width, Point start, Point end) : base(color, width)
        {
        ...
        public Square(Color color, float width, int x1, int x2, int y1, int y2) : base(color, width)
        {
        ...
        public override void Drawing(Graphics drawing)
        {
            ...
            drawing.DrawRectangle(Pen, Math.Min(StartPoint.X, EndPoint.X), Math.Min(StartPoint.Y, EndPoint.Y), width, width);
        }
    }
}
